...
DLC_PRELOAD=true
# colorburst override: factory-install termina into the release image so
# Crostini installs with NO DLC OTA. The colorburst update server only serves
# the OS payload, not DLCs, so the upstream DLC_FORCE_OTA path can never fetch
# termina. DLC_FACTORY_INSTALL bakes the built dlc.img into the stateful
# unencrypted/dlc-factory-images/<id>/ area at base_image_util.sh build time;
# at runtime dlcservice copies+verifies+mounts it locally
# (platform2/dlcservice/dlc_base.cc:667) and never calls update_engine.
# DLC_FORCE_OTA is dropped because it forces the OTA path we cannot serve.
# NOTE: requires termina-dlc in chromite/lib/dlc_allowlist.py DLC_FACTORY_INSTALL.
