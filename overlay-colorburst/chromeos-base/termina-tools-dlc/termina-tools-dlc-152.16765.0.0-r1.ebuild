...
# VM tools image should be ~75MB, and kernel ~15MB here we provide a generous
# buffer of 2x
DLC_PREALLOC_BLOCKS="$((200 * 256))"

DLC_PRELOAD=true
# colorburst override: factory-install into the release image so Crostini
# installs with NO DLC OTA (our update server serves only the OS payload, not
# DLCs). DLC_FACTORY_INSTALL bakes the dlc.img into stateful
# unencrypted/dlc-factory-images/<id>/; dlcservice mounts it locally
# (platform2/dlcservice/dlc_base.cc:667), never calling update_engine.
# DLC_FORCE_OTA is dropped as it forces the OTA path we cannot serve.
# NOTE: requires termina-tools-dlc in chromite/lib/dlc_allowlist.py.
...
src_install() {
	...
	# Only install vm tools image and kernel
	doins "${WORKDIR}"/*/vm_tools.img
	doins "${WORKDIR}"/*/vm_kernel
	dlc_src_install
}
